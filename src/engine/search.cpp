...
SearchResult Search::search(Position& pos, const SearchLimits& limits) {
    ...
    for (int depth = 1; depth <= limits.max_depth; ++depth) {
        ...
        emit_search_info(pos, limits, completed_depth, best_score);
    ...
    return {best_move, best_score, completed_depth, nodes_, extract_pv(pos)};
}

// ── PV extraction ───────────────────────────────────────────────────────────

std::vector<Move> Search::extract_pv(Position& pos) const {
    std::vector<Move> pv;
    pv.reserve(32);

    for (int ply = 0; ply < kMaxPly; ++ply) {
        TTEntry entry{};
        if (!tt_.probe(pos.key(), entry) || entry.best_move.is_null()) {
            break;
        }
    ...
    for (int i = static_cast<int>(pv.size()) - 1; i >= 0; --i) {
        pos.unmake_move(pv[i]);
    }

    return pv;
}

void Search::emit_search_info(Position& pos, const SearchLimits& limits, int depth, int score_cp) {
    ...
    info.nodes = nodes_;
    info.pv = extract_pv(pos);
    limits.info_callback(info);
}
